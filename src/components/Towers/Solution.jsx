        ...
        <div className="font-sans max-w-7xl mx-auto p-5">
            <header className="text-center mb-20">
                <h2 className="text-3xl sm:text-5xl lg:text-6xl mt-10 lg:mt-20 tracking-wide mb-3">
                    Our {" "}<span className="bg-gradient-to-r from-[#0038a8] to-white text-transparent bg-clip-text">
                    SOLUTIONS
                    </span>
                </h2>
            ...
            <nav className={`flex ${isNavOpen ? 'flex-col' : 'hidden'} lg:flex`}>
                {tabData.map(({ id, name }) => (
                    <button
                        key={id}
                        className={`px-4 py-2 text-[1.03rem] border-none rounded-md ${activeTab === id ? 'bg-[#007fff] text-white' : 'bg-gray-300'} mb-2 lg:mb-0 lg:mr-2`}
                        onClick={() => {
                            setActiveTab(id);
